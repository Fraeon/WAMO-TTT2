/*
 Copyright (c) 2012 Eric Hartin <me@fraeon.info>
 
 This file is part of "WAMO-TTT2", a modified version of "WAMO" by Rolando González Chévere <rolosworld@gmail.com>.

 "WAMO-TTT2" is free software: you can redistribute it and/or modify
 it under the terms of the GNU General Public License version 3
 as published by the Free Software Foundation.

 "WAMO-TTT2" is distributed in the hope that it will be useful,
 but WITHOUT ANY WARRANTY; without even the implied warranty of
 MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 GNU General Public License for more details.

 You should have received a copy of the GNU General Public License
 along with "WAMO-TTT2".  If not, see <http://www.gnu.org/licenses/>.
*/
  ...
  for( var i in move ) {
    if ( move.hasOwnProperty(i) ) {
      if ( i == 'move' || i == 'input' ) {
        ...
        arr.push( '<tr class="' + move[i] + '"><th>' + i + '</th><td>' + move[i] + '</td></tr>' );
      }
    }
  }
  ...
  function process() {
    ...
    Meta.array.$( atacante ).forEach( function( ataque ) {
      ...
      Meta.array.$( bloqueante ).forEach(function( castigo ) {
        if ( fast_startup - ataque.guard >= castigo.startup ) {
    ...
    var rows = ['<tr><th>Blocked move</th><th>Options</th></tr>'];

    Meta.array.$( castigos_lista ).forEach( function( castigos ) {
      var review = [],
      adv = castigos.ataque.guard * -1,
      ...
      Meta.array.$( castigos.castigos ).
        ...
        forEach( function( a ) {
          ...
          if (adv > a.startup && a.startup != 0 && a.stance != 'yes') {
            review.push( '<li class="best">' + ( a.move || a.input ) + '&nbsp;(&nbsp;' +  (a.damage) + '&nbsp;/&nbsp;' + (a.hit) + '&nbsp;)' + '</li>' );
          } else if ( adv == a.startup && a.startup != 0 && a.stance != 'yes' ) {
            review.push( '<li class="medium">' + ( a.move || a.input ) + '&nbsp;(&nbsp;' +  (a.damage) + '&nbsp;/&nbsp;' + (a.hit) + '&nbsp;)' + '</li>' );
          } else {
    ...
    Meta.array.$( links_lista ).forEach( function( ataque ) {
      ...
      Meta.array.$( ataque.contra_ataque ).
        ...
        forEach( function( a ) {
          ...
          // Ignore bad punishments
          if ( ( diff2 * -1 ) >= ( fast_startup - 1 ) ) {
            return;
          }
